...
A bare minimum crawler needs at least these components:
    i. **URL frontier**: To store the list of URLs to download and also prioritize which URLs should be crawled first.
    ii. **HTTP Fetcher**: To retrieve a web page from the server.
    iii. **Extractor**: To extract links from HTML documents.
